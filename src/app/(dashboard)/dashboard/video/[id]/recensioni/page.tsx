...
export default async function CorsoRecensioniPage({
  params,
  searchParams,
}: {
  ...
  const [recensioni, all] = await Promise.all([
    getRecensioniForOwner({ targetKey: "corso_id", targetId: id, stato }),
    getRecensioniForOwner({ targetKey: "corso_id", targetId: id }),
  ]);
  ...
  return (
    <div className="space-y-4">
      <RecensioniFilterTabs
        basePath={`/dashboard/video/${id}/recensioni`}
        active={stato}
        counts={counts}
      />
