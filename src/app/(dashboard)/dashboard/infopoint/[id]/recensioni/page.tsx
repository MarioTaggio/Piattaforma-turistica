...
export default async function AttrazioneRecensioniPage({
  params,
  searchParams,
}: {
  ...
  const [recensioni, all] = await Promise.all([
    getRecensioniForOwner({ targetKey: "attrazione_id", targetId: id, stato }),
    getRecensioniForOwner({ targetKey: "attrazione_id", targetId: id }),
  ]);
  ...
  return (
    <div className="space-y-4">
      <RecensioniFilterTabs
        basePath={`/dashboard/infopoint/${id}/recensioni`}
        active={stato}
        counts={counts}
      />
