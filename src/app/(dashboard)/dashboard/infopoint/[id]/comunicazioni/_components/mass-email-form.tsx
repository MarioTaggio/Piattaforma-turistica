...
export function MassEmailForm({ attrazioneId, destinatari }: Props) {
  ...
  function onSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (destinatari === 0) {
      toast.error("Nessun visitatore prenotato");
      return;
    }
    if (
      !confirm(
        `Inviare questa email a ${destinatari} visitator${destinatari === 1 ? "e" : "i"}?`,
      )
    )
      return;
    ...
    <form onSubmit={onSubmit} className="space-y-4" noValidate>
      <div className="space-y-1.5">
        ...
        <Input
          ...
          placeholder="Es. Cambio orario visita guidata"
          maxLength={120}
        />
      </div>
      <div className="space-y-1.5">
        ...
        <textarea
          ...
          rows={8}
          maxLength={2000}
          placeholder="Scrivi il messaggio da inviare ai visitatori prenotati…"
        ...
        <p className="text-[11px] text-muted-foreground">
          {messaggio.length}/2000 caratteri.
        </p>
      </div>
      <div className="flex items-center justify-between gap-3 border-t border-border pt-4">
        <p className="text-xs text-muted-foreground">
          ...
          visitator{destinatari === 1 ? "e" : "i"} con prenotazione attiva
        </p>
        <Button
          type="submit"
