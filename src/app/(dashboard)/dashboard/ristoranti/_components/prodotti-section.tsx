...
import { createProdotto, deleteProdotto } from "@/lib/gestore/ristoranti";
...
export function ProdottiSection({
  ristoranteId,
  prodotti,
}: {
  ristoranteId: string;
  ...
  async function onSubmit(values: ProdottoInput) {
    const r = await createProdotto(ristoranteId, values);
    if (r.error) {
      toast.error(r.error);
      return;
    }
  ...
  function onDelete(id: string) {
    ...
    startTransition(async () => {
      const r = await deleteProdotto(id, ristoranteId);
      if (r.error) {
        toast.error(r.error);
        return;
      }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-base font-semibold">Voci del menu</h3>
          <p className="text-xs text-muted-foreground">
            {prodotti.length === 0
              ? "Aggiungi la prima voce per pubblicare il menu del tuo ristorante."
              : `${prodotti.length} voc${prodotti.length === 1 ? "e" : "i"} pubblicat${prodotti.length === 1 ? "a" : "e"}.`}
          </p>
        </div>
        <Button
          type="button"
                ...
                <tr key={p.id}>
                  ...
                  <td className="px-4 py-2 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <MenuEditDialog prodotto={p} ristoranteId={ristoranteId} />
                      <Button
                        type="button"
                        size="sm"
                        variant="ghost"
        ...
        >
          <div className="grid gap-3 sm:grid-cols-3">
            <div className="space-y-1.5 sm:col-span-2">
              <Label>Nome</Label>
              <Input placeholder="Tagliatelle al ragù" {...register("nome")} />
              {errors.nome && (
            ...
            <div className="space-y-1.5">
              ...
              <Input placeholder="Primi, Dolci…" {...register("categoria")} />
            </div>
            <div className="space-y-1.5">
              <Label>Prezzo</Label>
              <Controller
