...
export default async function StrutturaRecensioniPage({
  params,
  searchParams,
}: {
  ...
  const [recensioni, all] = await Promise.all([
    getRecensioniForOwner({ targetKey: "struttura_id", targetId: id, stato }),
    getRecensioniForOwner({ targetKey: "struttura_id", targetId: id }),
  ]);
  ...
  return (
    <div className="space-y-4">
      <RecensioniFilterTabs
        basePath={`/dashboard/bnb/${id}/recensioni`}
        active={stato}
        counts={counts}
      />
