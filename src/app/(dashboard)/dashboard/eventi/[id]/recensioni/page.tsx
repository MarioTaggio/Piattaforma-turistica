...
export default async function EventoRecensioniPage({
  params,
  searchParams,
}: {
  ...
  const [recensioni, all] = await Promise.all([
    getRecensioniForOwner({ targetKey: "evento_id", targetId: id, stato }),
    getRecensioniForOwner({ targetKey: "evento_id", targetId: id }),
  ]);
  ...
  return (
    <div className="space-y-4">
      <RecensioniFilterTabs
        basePath={`/dashboard/eventi/${id}/recensioni`}
        active={stato}
        counts={counts}
      />
